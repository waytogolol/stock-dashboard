...
├─ 快取/          活的中間快取(tmp_*.pkl/json等,腳本引用路徑已全改成快取/tmp_*),gitignored
├─ 暫存/          一次性產物垃圾場(孤兒快取/跑批紀錄/一次性腳本),gitignored
├─ 封存/          研究筆記按日期歸檔 + 舊腳本
...
兩種方式的內容一致，五個分頁：
1. **題材跨市場比較** — 預設只顯示前10大「題材概念股」熱度分數排行(排除金融/消費/傳統產業等廣義分類)，選一個族群(被動元件/CPO等)看各國入榜公司明細，依排名熱度上色。熱度分數=該題材在每個國家的「台幣金額÷該國全部上榜公司台幣金額總和」百分比，五國加總而成，分數越高代表資金集中度越高。有第二次snapshot後會多顯示熱度分數Δ/金額Δ欄位，以及一週內熱度上升/下降前5名的橫條圖
2. **排行榜明細** — 篩選國家+題材的完整排行表，可點表頭排序
3. **公司歷史趨勢** — 選一家公司或一個主族群(題材)看排名/熱度分數隨時間變化(累積多次snapshot後才有趨勢可看)
4. **財報/法說會提醒** — 顯示`check_earnings.py`查到的美股財報日曆+台股法說會排程(只列觀察名單裡的公司)，依日期遠近上色(🔥3天內/🟠7天內)，附時間軸散點圖。app.py版本可直接按按鈕重新查詢；dashboard.html是靜態快照，要更新需重跑`check_earnings.py`再重跑`export_html.py`
5. **新聞/目標價** — 跟資金流向(熱度分數)的量化訊號互補的質化研究，每個題材附上新聞或外資投行(JPM/GS等)目標價，**全部人工搜尋並逐一驗證連結真實性，不是自動爬蟲**，連結點開都能看到對應內容。資料存在`theme_news.csv`，可用主族群/類型篩選
...
| `theme_news.csv` | 各題材新聞/目標價研究，人工搜尋+逐一驗證連結後手動整理，要新增資料直接編輯這個檔案再重跑`export_html.py` |
