# -*- coding: utf-8 -*-
"""台美隔夜題材連動·第二輪:題材獨漲組合精煉+可執行組合模擬+正式HTML報告(2026-08-05)。
...
承接: build_us_tw_overnight_link.py(首輪考卷,commit ac221a4,結論只在console)。首輪判決:
  ...
  ②題材獨漲(美題材原始>2%且SPX<0,~85題材日/年)開盤進5日+0.68%/10日+1.15%,12年11正
  ③創新高交乘單調增強(→+0.78%)僅多日持有有價值。
本卷=使用者交辦的後續: 獨漲×創新高×強題材子集組合精煉,並補產正式HTML報告上index。

═══ 本卷四個問題(寫程式前預先註冊) ═══
Q1 獨漲定義敏感度: 門檻(原始>2/2.5/3%)×SPX條件(<0 / <-0.5%),肉量與n的權衡,不掃參數只看穩不穩。
Q2 獨漲×創新高交乘: hi_frac(美股側當日創52週新高成員佔比)=0 / >0 / >=50%三層,首輪已見單調,獨漲內複驗。
Q3 獨漲×強題材子集: (a)使用者指定四題材[記憶體/CPO/PCB/被動元件](首輪全樣本挑的,in-sample要誠實標註)
   (b)防過擬合對照: 用前半段(2015-2020)獨漲表現排名取前4題材,只在後半段(2021-)評估=split-half準OOS。
   兩版並列,若(a)只贏在前半段=過擬合警訊。
Q4 可執行組合模擬(成員層,錢的視角): 獨漲事件日開盤買進題材全成員等權(排除開盤跳空>=+9%≈漲停鎖死
...
· 進場錨=開盤(9:00市價,獨漲子集首輪已驗證跳空不過衝);事件研究輸出k=1~60逐日CAR
  (含30/40/60≈1.5~3個月,2026-08-05使用者追問延長窗)。
· 命名(2026-08-05定名): 訊號=「題材獨漲」(美題材漲且SPX跌,舊名「口袋」);強度軸=「分歧度」;
  強獨漲=原始>2.5%且SPX<-0.5%(舊名「深口袋」)。
...
MIN_TW_MEMBERS = 2
K_LIST = [1, 2, 3, 5, 7, 10, 15, 20, 30, 40, 60]   # 2026-08-05使用者追問:延長到~3個月看效果衰減/累積
    ...
    for th in (0.02, 0.025, 0.03):
        for spx_th, spx_lab in ((0.0, "SPX<0"), (-0.005, "SPX<-0.5%")):
            ...
            r = {"lab": f"原始>{th * 100:.1f}%×{spx_lab}", "n": len(sub), "nyr": n_per_yr,
                 ...
                 "car20": sub["car_o20"].mean() * 100, "car40": sub["car_o40"].mean() * 100,
                 "car60": sub["car_o60"].mean() * 100, "ci5": ci_str(sub, "car_o5")}
            y, pos, tot = yearly_tbl(sub)
            r["yr"] = f"{pos}/{tot}"
            rows.append(r)
            print(f"  {r['lab']:<24} n={r['n']:>4}({n_per_yr:.0f}/年) 跳空{r['gap']:+.2f} "
                  f"oc{r['oc']:+.2f} CAR5={r['car5']:+.2f} CAR10={r['car10']:+.2f} "
                  f"CAR20={r['car20']:+.2f} CAR40={r['car40']:+.2f} CAR60={r['car60']:+.2f} "
                  f"CI5:{r['ci5']} 逐年{r['yr']}")
    return rows
...
# ======================================================================
def q2_newhigh(P):
    print("\n" + "=" * 80, "\nQ2 獨漲×創新高交乘(hi_frac=美股側當日創52週新高成員佔比)")
    ...
    for lab, mask in [("無人創新高(=0)", pkt["hi_frac"] == 0),
        ...
        print(f"  {lab:<20} n={len(s):>4} 跳空{r['gap']:+.2f} oc{r['oc']:+.2f} "
              f"CAR: " + " ".join(f"k{k}={curve[k]:+.2f}" for k in (1, 3, 5, 10, 20, 40, 60)) +
              f"  CI5:{r['ci5']} 逐年{r['yr']}")
    return rows, pkt


# ======================================================================
# 4. Q3 獨漲×強題材子集(a使用者四題材 b split-half準OOS)
# ======================================================================
def q3_subsets(P):
    print("\n" + "=" * 80, "\nQ3 獨漲×強題材子集")
    ...
    print(f"  前半段(2015-2020)獨漲排名前4題材(n>=5): {top4_first}")
...
# ======================================================================
def q_combo(P):
    print("\n" + "=" * 80, "\n組合精煉交乘格(獨漲×四題材×創新高,n會很薄,誠實列)")
    pkt = P[(P["sig_raw"] >= 0.02) & (P["spx"] < 0)]
    combos = [
        ("獨漲(基準)", pkt),
        ("獨漲×hi>0", pkt[pkt["hi_frac"] > 0]),
        ("獨漲×四題材", pkt[pkt["theme"].isin(USER4)]),
        ("獨漲×四題材×hi>0", pkt[pkt["theme"].isin(USER4) & (pkt["hi_frac"] > 0)]),
        ("獨漲×原始>3%", pkt[pkt["sig_raw"] >= 0.03]),
        ("獨漲×原始>3%×四題材", pkt[(pkt["sig_raw"] >= 0.03) & pkt["theme"].isin(USER4)]),
    ]
    rows = []
    for lab, s in combos:
        curve = car_curve(s)
        ...
        print(f"  {lab:<22} n={r['n']:>4}({r['nyr']:.0f}/年) oc{r['oc']:+.2f} "
              f"CAR5={curve[5]:+.2f} CAR10={curve[10]:+.2f} CAR20={curve[20]:+.2f} "
              f"CAR60={curve[60]:+.2f} CI5:{r['ci5']} 逐年{r['yr']}")
    return rows
...
def simulate(P, tw_mem, tw_open, tw_close, tai, event_mask, lab, cost_side, hold=HOLD):
    """獨漲事件日開盤買題材全成員等權,排除開盤跳空>=LOCK_GAP,持有至第hold日收盤。
...
def q4_portfolio(P, tw_mem, tw_open, tw_close, tai):
    ...
    deep_mask = (P["sig_raw"] >= 0.025) & (P["spx"] < -0.005)   # 強獨漲(Q1的高肉格,事件數~19/年)
    user4_mask = pkt_mask & P["theme"].isin(USER4)
    hi_mask = pkt_mask & (P["hi_frac"] > 0)
    sims = []
    for lab, mask, hold in [("全獨漲·持5", pkt_mask, 5),
                            ("全獨漲·持10", pkt_mask, 10),
                            ("強獨漲(>2.5%×SPX<-.5%)·持5", deep_mask, 5),
                            ("強獨漲·持10", deep_mask, 10),
                            ("強獨漲·持20", deep_mask, 20),
                            ("獨漲×四題材·持5", user4_mask, 5),
                            ("獨漲×hi>0·持5", hi_mask, 5),
                            ("獨漲×四題材×hi>0·持5", user4_mask & (P["hi_frac"] > 0), 5)]:
...
def write_report(P, recap, q1_rows, q2_rows, theme_rows, top4_first, q3_out, combo_rows, sims):
    ...
    for lab, mask in [("大跌<-2%", P["sig_raw"] <= -0.02),
                      ...
                      ("大漲>2%", P["sig_raw"] >= 0.02),
                      ("大漲>2%且SPX<0(=獨漲)", (P["sig_raw"] >= 0.02) & (P["spx"] < 0))]:
        s = P[mask]
        hl = " class='hl'" if "獨漲" in lab else ""
    ...
    q1_html = ("<table><tr><th>獨漲定義(分歧度)</th><th>n(次/年)</th><th>跳空</th><th>當日oc</th>"
               "<th>CAR5</th><th>CAR10</th><th>CAR20(1個月)</th><th>CAR40(2個月)</th><th>CAR60(3個月)</th>"
               "<th>CAR5月群CI</th><th>逐年正</th></tr>")
    for r in q1_rows:
        q1_html += (f"<tr><th>{r['lab']}</th><td>{r['n']}({r['nyr']:.0f})</td><td>{r['gap']:+.2f}</td>"
                    f"<td>{r['oc']:+.2f}</td><td>{r['car5']:+.2f}</td><td>{r['car10']:+.2f}</td>"
                    f"<td>{r['car20']:+.2f}</td><td>{r['car40']:+.2f}</td><td>{r['car60']:+.2f}</td>"
                    f"<td>{r['ci5']}</td><td>{r['yr']}</td></tr>")
    q1_html += "</table>"

    khead = "".join(f"<th>k{k}</th>" for k in K_LIST)
    q2_html = f"<table><tr><th>獨漲×創新高層</th><th>n</th><th>跳空</th><th>oc</th>{khead}<th>CAR5 CI</th><th>CAR10 CI</th><th>逐年</th></tr>"
    for r in q2_rows:
    ...
    tt_html = ("<table><tr><th>題材(★=使用者四題材)</th><th>獨漲n</th><th>oc</th><th>CAR5</th><th>CAR10</th></tr>")
    for r in theme_rows:
        star = "★" if r["theme"] in USER4 else ""
    ...
    base_sim = next((s for s in sims if s["lab"] == "全獨漲·持5" and s["cost"] == 1.0), None)
...
獨漲子集首輪已驗證「跳空不過衝」(台股開盤只定價美股大盤層,題材專屬訊號無大盤掩護時定價不足),
...
<li>可執行性: 普通大漲日開盤追買被跳空過衝殺死(跳空吃掉>100%,盤中回吐);<b>獨漲日(題材獨漲)是例外</b>——跳空溫和且盤中不回吐。</li>
<li>強弱題材: 記憶體最強(+0.57%當日cc),CPO/AI伺服器次之;EMS/網通/電信無效。</li>
</ul>

<h2>§1 Q1 獨漲定義敏感度(門檻×SPX條件)</h2>
<div class="note">不是掃參數選最好,是看肉量對定義擾動穩不穩+n的權衡。CI=月群bootstrap 95%。</div>
{q1_html}

<h2>§2 Q2 獨漲×創新高交乘</h2>
<div class="note">hi_frac=訊號日美股側成員創52週新高(收盤>=252日滾動高*0.999)佔比。首輪全樣本已見單調,此處獨漲內複驗。</div>
{q2_html}

<h2>§3 Q3 獨漲×強題材子集</h2>
<h3>分題材獨漲成績(全樣本,獨漲n>=8)</h3>
{tt_html}
<h3>子集比較(CAR逐日k=1~20,demean;絕對報酬並列=使用者目標函數第一層)</h3>
<div class="note">⚠(a)四題材是首輪<b>全樣本</b>看出來的強題材=in-sample挑選;(b)用前半段(2015-2020)獨漲排名
...
<div class="note">獨漲事件日<b>開盤市價買進題材全體台股成員等權</b>(排除開盤跳空>=+9%≈漲停鎖死買不到,
表列「排鎖」數),持有至第5或第10個交易日收盤(標籤註明);重疊事件日頻等權再平衡,無事件期空手(0%)。
「強獨漲」=原始>2.5%且SPX<-0.5%(Q1的高肉格,~19題材日/年)。
...
<ul>
<li><span class="verdict v-good">①獨漲主訊號成立且對定義穩健</span> 基準獨漲n={pkt_r['n']}
(~{pkt_r['n'] / n_years:.0f}題材日/年)CAR5={pkt_r['curve'][5]:+.2f}%/CAR10={pkt_r['curve'][10]:+.2f}%(demean),
CI5:{pkt_r['ci5']},逐年{pkt_r['yr']};Q1六種定義全部CI排0——且<b>分歧越大肉越厚</b>
(強獨漲=原始&gt;2.5%×SPX&lt;-0.5%: CAR5+1.44/CAR10+1.83,事件數~19/年)。真正的精煉軸是「分歧度」
(題材漲幅與大盤跌幅的極端程度),不是題材子集。</li>
<li><span class="verdict v-bad">②創新高交乘在獨漲內反轉(與首輪全樣本結論矛盾,誠實攤開)</span>
獨漲內hi=0組CAR5+0.85✓排0(逐年11/12) vs hi&gt;0組+0.33含0——機制一致可解釋: 創新高題材跳空大
(+0.45 vs +0.04)且盤中回吐(oc-0.21),「無人創新高的題材獨漲」才是定價不足最嚴重的角落。
但此反轉是<b>短窗現象</b>: k40起兩組拉平(hi&gt;=50%組k40+5.69 vs hi=0組+4.47)=創新高題材
只是頭幾天被跳空透支,1-2個月後追回——短持(5-10日)不要疊創新高,長持則無所謂。</li>
...
全獨漲·持5在0成本年化+24.3%/MDD-34.7%,1%來回成本直接翻負——年週轉~40次的結構性宿命
(與週級動能同教訓)。活口方向=<b>強獨漲+持5~10</b>+當「持股tilt/加碼觸發器」使用
(本來就要買的股票,挑獨漲日買),而非獨立資金線;精確數字見§5表。</li>
<li><span class="verdict v-good">⑤事件窗延長判決(2026-08-05使用者追問): 效應不是5天噴完,
是1~3個月持續累積——但組合最佳持有期仍是5~10日</span>
CAR曲線一路向上不反轉(基準獨漲k5+0.68→k20+2.00→k40+4.61→k60+6.14;強獨漲k60+7.25),
形狀=前5-10天「事件層錯價修復」+之後「題材動能延續層」(⚠k60窗平均會撞到下一次獨漲事件,
長窗含後續事件貢獻,見限制⑦)。<b>但持有期不該跟著拉長</b>: 強獨漲·持20(0.5%成本)年化+14.6%/
MDD-42.8%/Calmar0.34,全面輸給持5(+19.9%/-24.8%/0.80)與持10(+22.0%/-32.3%/0.68)——
拉長持有讓事件疊加層變成準買進持有,曝險62%把beta回檔全吃進來,MDD爆掉(展望理論第一優先=MDD小)。
長窗肉的正確吃法=「本來就打算持有一季的部位挑獨漲日進場」(進場時機工具),不是把overlay持有期拉長。</li>
</ul>

<h2>已知限制(誠實聲明)</h2>
<div class="note">
①四題材子集是首輪全樣本觀察後指定=in-sample;split-half對照只有~6年×2段,檢定力有限,結論以「候選」定位。
②組合模擬的開盤成交假設: 獨漲日跳空溫和(表列),但成員層仍排除跳空>=+9%;0.5-1%來回成本涵蓋一般滑價,
...
⑥強獨漲(>2.5%×SPX&lt;-0.5%)是Q1六格中最深的格=溫和的in-sample選格(緩解: 六格全部CI排0且深度單調,
是機制一致的結構非cherry-pick);已抽查逐年分布=9/12年CAR5正、肉分散於2018-2021/2024-2026多段非單年撐盤;
但事件組成48%集中在化合物半導體+電池/儲能兩個台股側成員僅3-5檔的小題材,組合分散度有限,
單一題材熄火會直接砍半事件數——live使用時要留意題材組成漂移。
⑦長窗CAR(k=30/40/60)的事件窗遠長於月群集群單位且重疊嚴重,表中長窗數字只看形狀(累積/衰減/反轉),
不做單格顯著性判讀;且k60窗內平均會撞到下一次獨漲事件,長窗CAR部分含「後續事件」的貢獻,
是「訊號家族的自然重疊」而非單一事件純效應。</div>
