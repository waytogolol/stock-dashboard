...
def main():
    ...
    pub_dates = [(m + pd.Timedelta(days=12)).strftime("%Y-%m-%d") for m in hi2_m.index]
    hi2_pub = hi2_m.copy()
...
<li><span class="verdict v-good">②同引擎對決: 雙新高月頻=主幹</span> (b)年化+69.2%/MDD-36.2%/
<b>Calmar1.91</b>/夏普1.70/勝率58%/PF3.37,全面壓過(a)三重門檻(+37.1%/Calmar1.13)與TAIEX(+18.0%/0.57);
(c)題材版年化+78%最高但並行僅5.7檔/MDD-44.7%——集中度換報酬,依展望理論排序(MDD優先)<b>全池版(b)
勝出</b>(與九宮格卷「雙新高讓非題材也活」一致)。(e)交集+48.3%但n=163/並行3.8檔太薄;(d)聯集被(a)的
大量事件稀釋。</li>
<li><b>③實務配置(候選)</b>: 主幹=雙新高月頻(P1×R1,全池,每月13日調倉,H40)≈11檔並行;
衛星=三重門檻(事件驅動,並行33檔=分散載體,吃「剛突破」的另一群股票);兩者重疊自然去重。
統一掃描器按此設計。</li>
<li><span class="verdict v-warn">④誠實保留</span> (b)+69%年化是「意外好」等級數字——與九宮格事件層
+8.90%/40日算術一致(非bug),但2018-2026成長股時代+月頻狀態薄組合(11檔)+名單偏誤(題材版)都在推高;
候選層,watch統一掃描器live累積樣本外才算數;除權息未還原=此數字反而低估(慰藉項)。</li>
</ul>
<h2>已知限制</h2>
