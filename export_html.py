...
<div class="tab-content active" id="tab0">
  ...
  <div class="hint"><span id="rrgRegime" style="font-weight:bold"></span><br>
  <b>交易員速讀,看兩樣就好</b>:①上行徽章=現在的regime與打法 ②圖上<b>★金框</b>=現在該盯的題材(「象限+點色」兩條件已代判,滑鼠停上去看該做什麼與抱多久)。
  ⚠跌破季線時,右上「領先」象限整區避開。
  點色:<span style="color:#e74c3c">●增漲</span> <span style="color:#f39c12">●增平</span> <span style="color:#a569bd">●增跌</span>
  <span style="color:#f5b7b1">●縮漲</span> <span style="color:#95a5a6">●縮平</span> <span style="color:#5dade2">●縮跌</span>(量=成交佔比變化/價=成員週報酬);
  軸:橫=相對大盤強弱、縱=加速或減速,右上領先→右下轉弱→左下落後→左上轉強,順時針輪。
  操作:「行動優先」只亮值班格/「聚焦」看單一題材/「▶週序動畫」看近6週怎麼輪。
  <b>所有規則、數據、持有期、警語 → 研究報告/research_heat_flow.html(開頭就是交易員速查表)</b>;觀察層非上板規則;維運=build_heat_flow.py(已入update_all)。</div>
  <div class="controls">
...
function renderWarRoom() {
  ...
  if (rg.above60) {
    ...
    dos.push("<b>買什麼</b>：①<b>落後象限的紅點●增漲</b>＝跌很深後第一次放量上漲，可抱1~2個月不用急著跑" +
             (!rg.above20 ? "（大盤越爛這招歷史上越有效）" : "") +
             "；②<b>轉強象限的淡紅點●縮漲</b>＝安靜爬升，只做1~2週。兩個條件（象限+點色）要同時成立，首頁🧭地圖的★金框已經幫你判好。");
  ...
  const rows = allRows.filter(function(r) { return String(r.code).length === 4; }).map(function(r) {
    ...
    return {
      ...
      "第3日值": (function() {
        ...
        if (r.tv3r) {
          s += "(#" + r.tv3r + ")";
          if (bull60 && r.tv3r <= 300) {
            s = "💰" + s;
            // 疊加觀察格(2026-07-29使用者裁示上板,掛起等live): 💰∧d4w>0 / 💰∧題材成員
            if (r.d4w !== null && r.d4w !== undefined && r.d4w > 0) s += "×d4w";
            if (r.theme) s += "×題";
          }
